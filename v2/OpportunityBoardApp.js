(function(global) {
  ...
  function buildValidationSummary(store) {
    var messages = [
      store.Get("accountError"),
      store.Get("stageError"),
      store.Get("valueError"),
      store.Get("productError"),
      store.Get("ownerError")
    ].filter(function(message) {
      return !!message;
    });

    store.Set("validationSummary", messages.length ? "Please fix: " + messages.join(" | ") : "");
  }
  ...
  function OpportunityEditorDialog(store, onSave) {
    ...
    validation.BindSummary(store, "validationSummary");
  ...
  function OpportunityBoardPage() {
    ...
    function updateBoard() {
      ...
      opportunities.forEach(function(record) {
        ...
        rowCard.Title = record.account;
        rowCard.Padding = 12;

        var rowGrid = new JOG.Grid();
        rowGrid.Name = "opportunityRowGrid" + record.id;
        rowGrid.Columns = ["180px", "110px", "120px", "150px", "130px", "1fr", "170px"];
        rowGrid.ColumnGap = 12;
        rowGrid.RowGap = 6;
