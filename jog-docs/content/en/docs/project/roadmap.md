...
---
> Generated from `doc/roadmap.md`. Edit the source file, then rerun `node jog-docs/scripts/sync-docs.mjs`.
...
- a concrete design spec now exists for a Lexical-based proof control in [doc/lexical-control-spec.md](https://github.com/jjanuszczak/jog/blob/main/doc/lexical-control-spec.md), starting with a `LexicalPlainTextBox` third-party input wrapper
